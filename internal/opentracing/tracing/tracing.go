...
// opentracing tags
var (
	AuroraTag        = opentracing.Tag{Key: string(opentracing_ext.Component), Value: "aurora"}
	WorkflowGroupTag = opentracing.Tag{Key: "aurora.workflow", Value: "group"}
	WorkflowChordTag = opentracing.Tag{Key: "aurora.workflow", Value: "chord"}
	WorkflowChainTag = opentracing.Tag{Key: "aurora.workflow", Value: "chain"}
)
...
func StartSpanFromHeaders(headers tasks.Headers, operationName string) opentracing.Span {
	...
	// For clarity add the aurora component tag.
