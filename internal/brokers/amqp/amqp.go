	...
	if !ok {
		...
		go func() {
			...
			case err = <-conn.errorchan:
				log.Runtime().Infof("Error occurred on queue: %s. Reconnecting", queueName)
				b.connectionsMutex.Lock()
				...
				if err != nil {
					log.Runtime().Errorf("Failed to reopen queue: %s.", queueName)
				}
			case <-conn.cleanup:
				return
			}
...
func (b *Broker) CloseConnections() error {
	...
	for key, conn := range b.connections {
		if err := b.Close(conn.channel, conn.connection); err != nil {
			log.Runtime().Errorf("Failed to close channel")
			return nil
		}
		close(conn.cleanup)
...
func (b *Broker) consumeOne(delivery amqp.Delivery, taskProcessor iface.TaskProcessor, ack bool) error {
	...
	if !b.IsTaskRegistered(signature.Name) {
		requeue = true
		log.Runtime().Infof("Task not registered with this worker. Requeing message: %s", delivery.Body)
	...
	log.Runtime().Debugf("Received new message: %s", delivery.Body)
...
func (b *Broker) TestConnect() error {
	connection, channel, _, _, _, err := b.Connect(
		b.GetConfig().Broker,
		b.GetConfig().MultipleBrokerSeparator,
		b.GetConfig().TLSConfig,
		b.GetConfig().AMQP.Exchange,     // exchange name
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to connect to queue %s", b.GetConfig().DefaultQueue)
	}
	_ = b.Close(channel, connection)
	return nil
}
